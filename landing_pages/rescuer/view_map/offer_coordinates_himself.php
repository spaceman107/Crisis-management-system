...
transaction.time_of_acceptance
    ...
    WHERE user.user_id IN (
        ...
        WHERE (transaction.status= 'ACCEPTED' AND transaction.type='OFFER'AND user.user_id = $logged_in_user_id)
    )
";
    $OfferCoordinatesResult = mysqli_query($con, $OfferCoordinatesQuery);

    $OfferHimselfCoordinates = [];
    while ($row = mysqli_fetch_assoc($OfferCoordinatesResult)) {
        $OfferHimselfCoordinates[] = $row;
    }

    header('Content-Type: application/json');
    echo json_encode($OfferHimselfCoordinates);
} else {
