...
function rescuer_transaction(userId, transactionId) {
...
fetch('offer_coordinates.php')
    ...
    .then(OfferCoordinates => {
        ...
        OfferCoordinates.forEach(item => {
           ...
           var markerOffer =  L.marker([parseFloat(item.lat), parseFloat(item.lng)], { icon: markerOffer })
                ...
                .bindPopup(
                    ...
                    "<br> Status : Pending" );
                    markerOffer.on('click', function () {
                        // Call the common function with the user_id and transaction_id
                        rescuer_transaction(item.user_id, item.transaction_id);
                    });
