...
$RequestAcceptedCoordinatesQuery = " SELECT x_coordinate AS lat, y_coordinate AS lng ,   
user.first_name ,
user.last_name,
user.phone, 
...
vehicle.vehicle_name
    ...
    WHERE (transaction.type = 'REQUEST'AND transaction.status='ACCEPTED')";
$RequestAcceptedCoordinatesResult = mysqli_query($con, $RequestAcceptedCoordinatesQuery);

$RequestAcceptedCoordinates = [];
while ($row = mysqli_fetch_assoc($RequestAcceptedCoordinatesResult)) {
    $RequestAcceptedCoordinates[] = $row;
}
    header('Content-Type: application/json');
echo json_encode($RequestAcceptedCoordinates);
